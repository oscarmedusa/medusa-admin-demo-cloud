...
function prepareListQuery(validated, queryConfig = {}) {
    ...
    let { allowed = [], defaults = [], defaultFields = [], defaultLimit, allowedFields = [], allowedRelations = [], defaultRelations = [], isList, } = queryConfig;
    ...
    let allFields = new Set(defaultFields);
    ...
    if (allowedFields.length) {
        ...
        [...allFields, ...Array.from(starFields)].forEach((field) => {
            ...
            const fieldStartsWithAllowedField = allowedFields.some((allowedField) => field.startsWith(allowedField));
    ...
    if (allowedRelations.length && expand) {
        ...
        const notAllowedRelations = (0, utils_1.getSetDifference)(allRelations, allAllowedRelations);
        if (allRelations.size && notAllowedRelations.size) {
            throw new utils_1.MedusaError(utils_1.MedusaError.Types.INVALID_DATA, `Requested fields [${Array.from(notAllowedRelations).join(", ")}] are not valid`);
        }
    }
