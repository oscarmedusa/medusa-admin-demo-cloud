...
type ExtractFiltersOperators<MaybeT, Lim extends number = Depth[2], Exclusion extends string[] = [], T = TypeOnly<MaybeT>> = {
    [Key in keyof T]?: Key extends Exclusion[number] ? never : Key extends ExcludedProps ? never : T[Key] extends string | number | boolean | Date ? T[Key] | OperatorMap<T[Key] | T[Key][]> : T[Key] extends Array<infer R> ? TypeOnly<R> extends {
        __typename: any;
