...
const LinkModulePackage = definitions_1.MODULE_PACKAGE_NAMES[utils_1.Modules.LINK];
async function loadModules(args) {
    const { modulesConfig, sharedContainer, sharedResourcesConfig, migrationOnly = false, loaderOnly = false, workerMode = "server", } = args;
    const allModules = {};
    const modulesToLoad = [];
    for (const moduleName of Object.keys(modulesConfig)) {
        ...
        if (declaration.scope === types_1.MODULE_SCOPE.INTERNAL && !declaration.resources) {
...
async function initializeLinks({ config, linkModules, injectedDependencies, moduleExports, }) {
    try {
        ...
        return {
            remoteLink: new remote_link_1.RemoteLink(),
            linkResolution,
            getMigrationPlanner,
        };
    }
...
async function MedusaApp_({ sharedContainer, sharedResourcesConfig, servicesConfig, modulesConfigPath, modulesConfigFileName, modulesConfig, linkModules, remoteFetchData, injectedDependencies = {}, migrationOnly = false, loaderOnly = false, workerMode = "server", } = {}) {
    ...
    const modules = modulesConfig ??
        (await (0, utils_1.dynamicImport)(await (modulesConfigPath ??
    ...
    return {
        ...
        modules: allModules,
        link: remoteLink,
        query: (0, remote_query_1.createQuery)(remoteQuery), // TODO: rm any once we remove the old RemoteQueryFunction and rely on the Query object instead,
        entitiesMap,
        gqlSchema: schema,
        notFound,
        runMigrations,
