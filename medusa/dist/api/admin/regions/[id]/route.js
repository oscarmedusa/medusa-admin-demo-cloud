...
const GET = async (req, res) => {
    const region = await (0, helpers_1.refetchRegion)(req.params.id, req.scope, req.remoteQueryConfig.fields);
    if (!region) {
...
const POST = async (req, res) => {
    ...
    const region = await (0, helpers_1.refetchRegion)(result[0].id, req.scope, req.remoteQueryConfig.fields);
    res.status(200).json({ region });
};
exports.POST = POST;
