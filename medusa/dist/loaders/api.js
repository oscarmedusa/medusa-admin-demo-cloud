...
exports.default = async ({ app, container, plugins }) => {
    ...
    try {
        await new http_1.RoutesLoader({
            app: app,
            sourceDir: sourcePaths,
        }).load();
    }
    catch (err) {
        throw Error("An error occurred while registering API Routes. See error in logs for more details.", { cause: err });
    }
    return app;
};
