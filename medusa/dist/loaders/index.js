...
async function loadEntrypoints(plugins, container, expressApp, rootDirectory) {
    ...
    await (0, admin_1.default)({ app: expressApp, configModule, rootDirectory });
...
async function initializeContainer(rootDirectory) {
    (0, config_1.configLoader)(rootDirectory, "medusa-config.js");
...
exports.default = async ({ directory: rootDirectory, expressApp, }) => {
    ...
    const configModule = container.resolve(utils_1.ContainerRegistrationKeys.CONFIG_MODULE);
    const plugins = (0, resolve_plugins_1.getResolvedPlugins)(rootDirectory, configModule, true) || [];
    ...
    const { onApplicationStart, onApplicationShutdown, onApplicationPrepareShutdown, gqlSchema, } = await new framework_1.MedusaAppLoader().load();
