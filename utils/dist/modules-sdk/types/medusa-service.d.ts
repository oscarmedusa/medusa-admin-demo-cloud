import { Constructor, Context, FindConfig, IDmlEntity, InferEntityType, Pluralize, RestoreReturn, SoftDeleteReturn } from "@medusajs/types";
...
export type ModelConfigurationsToConfigTemplate<T extends ModelEntries> = {
    [Key in keyof T]: {
         ...
         * @deprecated
         */
        singular: T[Key] extends {
            singular: string;
        } ? T[Key]["singular"] : Key;
        /**
         * @deprecated
         */
        plural: T[Key] extends {
            plural: string;
        } ? T[Key]["plural"] : Pluralize<Key & string>;
    };
};
/**
...
export type AbstractModuleService<TModelsDtoConfig extends Record<string, any>> = {
    [TModelName in keyof TModelsDtoConfig as `retrieve${ExtractSingularName<TModelsDtoConfig, TModelName>}`]: (id: string, config?: FindConfig<any>, sharedContext?: Context) => Promise<TModelsDtoConfig[TModelName]["dto"]>;
} & {
    [TModelName in keyof TModelsDtoConfig as `list${ExtractPluralName<TModelsDtoConfig, TModelName>}`]: (filters?: any, config?: FindConfig<any>, sharedContext?: Context) => Promise<TModelsDtoConfig[TModelName]["dto"][]>;
} & {
