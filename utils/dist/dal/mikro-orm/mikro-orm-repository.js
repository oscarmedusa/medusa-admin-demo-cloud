...
function mikroOrmBaseRepositoryFactory(entity) {
    class MikroOrmAbstractBaseRepository_ extends MikroOrmBaseRepository {
        ...
        async delete(filters, context) {
            const manager = this.getActiveManager(context);
            await manager.nativeDelete(entity, filters);
        }
        async find(options, context) {
            ...
            return await manager.find(entity, findOptions_.where, findOptions_.options);
        }
        async findAndCount(findOptions = { where: {} }, context = {}) {
            ...
            return await manager.findAndCount(entity, findOptions_.where, findOptions_.options);
        }
        async upsert(data, context = {}) {
            const manager = this.getActiveManager(context);
            const primaryKeys = MikroOrmAbstractBaseRepository_.retrievePrimaryKeys(entity);
        ...
        async upsertWithReplace(data, config = {
            ...
            let { orderedEntities: upsertedTopLevelEntities, performedActions: performedActions_, } = await this.upsertMany_(manager, entity.name, toUpsert);
