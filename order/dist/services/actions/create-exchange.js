...
function createExchangeAndReturnEntities(em, data, order) {
    const exchangeReference = em.create(_models_1.OrderExchange, {
    ...
    const returnReference = em.create(_models_1.Return, {
        order_id: data.order_id,
        order_version: order.version,
...
async function processAdditionalItems(em, service, data, order, exchangeReference, actions, sharedContext) {
    ...
    data.additional_items?.forEach((item) => {
        ...
        if (hasItem) {
            ...
            additionalItems.push(em.create(_models_1.OrderExchangeItem, {
                item_id: item.id,
                quantity: item.quantity,
                note: item.note,
                metadata: item.metadata,
            ...
            additionalNewItems.push(em.create(_models_1.OrderExchangeItem, {
                quantity: item.quantity,
