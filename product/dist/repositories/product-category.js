...
class ProductCategoryRepository extends utils_1.DALUtils.MikroOrmBaseTreeRepository {
    ...
    async buildProductCategoriesWithTree(include, productCategories, findOptions = { where: {} }, context = {}) {
        ...
        let relationIndex = findOptions.options?.populate?.indexOf("parent_category");
        ...
        const categoriesInTree = await this.serialize(await manager.find(_models_1.ProductCategory, where, options));
    ...
    async delete(ids, context = {}) {
        ...
        await manager.nativeDelete(_models_1.ProductCategory, { id: ids }, {});
    }
    async softDelete(ids, context = {}) {
        const manager = super.getActiveManager(context);
        await this.baseDelete(ids, context);
        const categories = await Promise.all(ids.map(async (id) => {
            const productCategory = await manager.findOne(_models_1.ProductCategory, {
                id,
            });
            if (!productCategory) {
    ...
    async restore(ids, context = {}) {
        const manager = super.getActiveManager(context);
        const categories = await Promise.all(ids.map(async (id) => {
            const productCategory = await manager.findOneOrFail(_models_1.ProductCategory, {
                id,
            });
    ...
    async baseDelete(ids, context = {}) {
        ...
        await Promise.all(ids.map(async (id) => {
            const productCategory = await manager.findOne(_models_1.ProductCategory, { id }, {
    ...
    async create(data, context = {}) {
        ...
        const categories = await Promise.all(data.map(async (entry, i) => {
            const categoryData = { ...entry };
            const siblingsCount = await manager.count(_models_1.ProductCategory, {
            ...
            if (parentCategoryId) {
                const parentCategory = await manager.findOne(_models_1.ProductCategory, parentCategoryId);
                if (!parentCategory) {
            ...
            return manager.create(_models_1.ProductCategory, categoryData);
        }));
        manager.persist(categories);
        return categories;
    }
        ...
        const categories = await Promise.all(data.map(async (entry, i) => {
            ...
            if ((0, utils_1.isDefined)(categoryData.parent_category_id) &&
                ...
                if (categoryData.parent_category_id === null) {
                    ...
                    const newParentCategory = await manager.findOne(_models_1.ProductCategory, categoryData.parent_category_id);
    ...
    async rerankSiblingsAfterDeletion(manager, removedSibling) {
        const affectedSiblings = await manager.find(_models_1.ProductCategory, {
